package com.uiteco.contentPanels.CauLacBo.SlideShow;

import com.uiteco.contentPanels.CauLacBo.JpanelRound;
...
public class Slideshow extends JLayeredPane {
    ...
    private boolean IsOnTimer = true;

    public Slideshow() {
        Start();
    }
    
    public Slideshow(boolean IsOnTimer) {
    ...
    public void initSlideshow(ArrayList<String> ListImageUrl) {
        if (ListImageUrl.size() >= 2) {
            for (String imageurl : ListImageUrl) {
                JpanelRound imagepanel = new JpanelRound(imageurl);
                
                imagepanel.setVisible(false);
                panel.add(imagepanel, "pos 0 0 0 0");
            }
            if (panel.getComponentCount() > 0) {
                componentShow = panel.getComponent(0);
                componentShow.setVisible(true);
